...
class OverviewKanbanTest < ActiveSupport::TestCase
  ...
  context "#extract_highest_priority_issue" do
    ...
    should "return the issue with the highest priority (by position)" do
      @medium = Issue.custom_generate!(:author => @user, :project => @project, :priority => medium_priority).reload
      @high = Issue.custom_generate!(:author => @user, :project => @project, :priority => high_priority).reload
      @medium2 = Issue.custom_generate!(:author => @user, :project => @project, :priority => medium_priority).reload
    ...
    context "with subissues_take_higher_priority set to true" do
      ...
      should "pick the highest priority of subissues" do
        ...
        @high = Issue.custom_generate!(:author => @user, :project => @project, :priority => high_priority).reload
        @medium2 = Issue.custom_generate!(:author => @user, :project => @project, :priority => medium_priority).reload
        @medium_subissue = Issue.custom_generate!(:author => @user, :project => @project, :priority => medium_priority, :parent_issue_id => @medium.id).reload
        @high_subissue = Issue.custom_generate!(:author => @user, :project => @project, :priority => high_priority, :parent_issue_id => @medium.id).reload
